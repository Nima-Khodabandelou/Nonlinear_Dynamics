...
omega0=sqrt(a2);
figure(1)
%  qddot + a(1)*qdot + a(2)*q + a(3)*q^2 + a(4)*q^3 + a(5) = 0
qmax=0.01;
tmax=10;
time=0:0.01:tmax;
...
%c1=-(1/12)*dum1+(1/3)*dum2-(1/3)*delta2/delta1-(1/2*i)*sqrt(3)*((1/6)*dum1+(2/3)*dum2);
a=c1*exp(-a1*time/2);
beta=(c1^2*time*(9*a4*omega0^2-10*a3^2))/(24*omega0^3)-time*(a3*a5)/omega0^3;
A=(a/2).*exp(i*beta);
Abar=(a/2).*exp(-i*beta);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
q0=A.*exp(i*omega0*time)+Abar.*exp(-i*omega0*time);
q1=(-2*a3.*A.*Abar-a5)/a2+(a3.*A.^2/(3*a2)).*exp(2*i*omega0*time)+(a3.*Abar.^2/(3*a2)).*exp(-2*i*omega0*time);
...
ode=@(t,w_h)[w_h(2);
                   ...
                   coeffs(3)*w_h(1)^2+coeffs(4)*w_h(1)^3+coeffs(5)];
